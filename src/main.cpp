...
void init_freertos() {
    ...
    inference_queue = xQueueCreate(INFERENCE_QUEUE_SIZE, sizeof(float));
    if (sensor_queue == NULL) {
    ...
    xTaskCreate(inference_task, "InferenceTask", 256, NULL, 2, NULL);
    xTaskCreate(mqtt_task, "MQTTTask", 512, NULL, 2, NULL);
